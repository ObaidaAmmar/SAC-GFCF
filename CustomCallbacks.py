...
class SaveOnBestTrainingRewardCallback(BaseCallback):
    ...
    def __init__(self, check_freq: int, log_dir: str, verbose=1):
        ...
        #self.custom_writer = SummaryWriter("./counterfactuals_/SAC_3_Rew/")
    ...
    '''
    def _on_step(self) -> bool:
        # Increment the steps in the current episode
        self.steps_in_episode += 1
        self.total_reward += self.locals['rewards'][0]

        if self.locals['dones'][0]:
            # Normalize the total reward by the episode length
            normalized_reward = self.total_reward / self.steps_in_episode

            # Log the normalized reward to TensorBoard (under custom metrics)
            self.custom_writer.add_scalar("ep_rew_mean", normalized_reward, self.num_timesteps)
    ...
    #def _on_training_end(self):
        # Close writers when training ends
